...
public class PersianRug {
	...
	private static void persianRug(
			...
			if (north >= (palette.length - 1)/4) {
				north = 0;
			}
			if (east >= (palette.length - 1)/4) {
				east = 0;
			}
			if (south >= (palette.length - 1)/4) {
				south = 0;
			}
			if (west >= (palette.length - 1)/4) {
				west = 0;
			}
		
			int lineColorIndex = north + east + south + west + 1;
			
			StdDraw.setPenColor(palette[lineColorIndex]);
		
			StdDraw.line(llx + size/2, lly, llx + size/2, lly + size); // vert
			StdDraw.line(llx, lly + size/2, llx + size, lly + size/2); // horiz
		
		if (size > 0.01) {
			persianRug(palette, llx, lly, size/2, lineColorIndex, lineColorIndex, south, west); // bottom left
			persianRug(palette, llx + size/2, lly, size/2, lineColorIndex, east, south, lineColorIndex); // bottom right
			persianRug(palette, llx, lly + size/2, size/2, north, lineColorIndex, lineColorIndex, west); // top left
			persianRug(palette, llx + size/2, lly + size/2, size/2, north, east, lineColorIndex, lineColorIndex); // top left
